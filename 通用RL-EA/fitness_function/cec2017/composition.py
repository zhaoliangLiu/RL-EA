...
def _calc_w(x, sigma):
    ...
    w = np.sum(x*x, axis=1)
    nzmask = w != 0
    w[nzmask] = ((1.0/(w + 1e-5))**0.5)[nzmask] * np.exp(-w / (2.0*nx*sigma*sigma) + 1e-5)[nzmask]
    w[~nzmask] = float('inf')
    return w
...
def f21(x, rotations=None, shifts=None):
    ...
    return _composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) 
...
def f29(x, rotations=None, shifts=None, shuffles=None):
    ...
    return _compose_hybrids(x, rotations, shifts, shuffles, funcs, sigmas, offsets, biases) 
